...
func (w *wizard) makeGenesis() {
	...
	fmt.Println(" 3. HotStuff - scalable-hotstuff")
	...
	// BLS-Upgrade: settings for the scalable hotstuff consensus
	case choice == "3":
		...
		if checkString == "y" {
			...
			genesis.Config.HotStuff.Test = false
		}
		if genesis.Config.HotStuff.Test {
			fmt.Println()
			fmt.Println("A test chain with HotStuff is about to be activated.")
			fmt.Println("Default Config:")
			fmt.Println("blockPeriod = 5 seconds")
			fmt.Println("gasLimit = 47000000")
			fmt.Println("The speaker policy: Round-robin")
			fmt.Println("New blocks/Views are broadcasted once a response is received by the speaker")
			genesis.Config.HotStuff.Period = uint64(5)
		} else {
			fmt.Println()
			fmt.Println("How many seconds should blocks take? (default = 5 seconds)")
			genesis.Config.HotStuff.Period = uint64(w.readDefaultInt(5))

			fmt.Println()
			fmt.Println("Please decide the amount of gasLimit (default = 47000000)")
			genesis.GasLimit = uint64(w.readDefaultInt(47000000))

			fmt.Println()
			fmt.Println("Please decide the speaker policy (0: Round-robin(default), 1: Sticky, 2: VRF)")
			for {
				option := uint64(w.readDefaultInt(0))
				if option <= 2 {
					genesis.Config.HotStuff.SpeakerPolicy = option
					break
				} else {
					fmt.Println("Please decide the speaker policy (0: Round-robin(default), 1: Sticky, 2: VRF)")
					continue
		...
		signers = nil
		for {
			// Read the address of the miners
			address := w.readAddress()
			if address != nil {
				signers = append(signers, *address)
			} else {
				if len(signers) < int(params.MinimumMiner) {
					fmt.Printf("The minimum number of miners is %v.", params.MinimumMiner)
					continue
				}
				fmt.Println("Miner list: ")
