...
var (
	...
	// BLS-Upgrade settings
	DeveloperHotStuffFlag = cli.BoolFlag{
		Name:  "hsdev",
		Usage: "Ephemeral hotstuff network with a few pre-funded developer accounts (>4), mining enabled",
	}
	DeveloperHotStuffPeriodFlag = cli.IntFlag{
		Name:  "hsdev.period",
		Usage: "Block period to use in hotstuff-developer mode (must be greater than or equals to 5)",
	}
	// /BLS-Upgrade
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	// BLS-Upgrade: add new flags for hotstuff settings
	CheckExclusive(ctx, DeveloperHotStuffFlag, DeveloperFlag, LegacyTestnetFlag, RopstenFlag, RinkebyFlag, GoerliFlag)
	CheckExclusive(ctx, LightLegacyServFlag, LightServeFlag, SyncModeFlag, "light")
	CheckExclusive(ctx, DeveloperFlag, ExternalSignerFlag) // Can't use both ephemeral unlocked and external signer
	CheckExclusive(ctx, DeveloperHotStuffPeriodFlag, DeveloperPeriodFlag)
	// /BLS-Upgrade
	...
	// BLS-Upgrade: the following setting is added to interpret the behaviour of "--hsdev".
	case ctx.GlobalBool(DeveloperHotStuffFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 25052
		}
		// // Create new developer account or reuse existing one
		// developers := make([]accounts.Account, 0, params.MaximumMiner) // We set the maximum number of miners to 100
		// developersAddr := make([]common.Address, 0, params.MaximumMiner)

		// if accs := ks.Accounts(); len(accs) > 0 {
		// 	for _, acc := range accs {
		// 		developers = append(developers, acc)
		// 	}
		// }
		// for {
		// 	if len(developers) >= int(params.MinimumMiner) {
		// 		break
		// 	} else {
		// 		if developer, err := ks.NewAccount(""); err == nil {
		...
		// 		Fatalf("Failed to unlock developer account: %v", err)
		// 	}
		// 	developersAddr = append(developersAddr, developer.Address)
		// 	log.Info("Using developer account", "index", i+1, "address", developer.Address)
		// }

		// cfg.Genesis = core.DeveloperHotStuffGenesisBlock(uint64(ctx.GlobalInt(DeveloperHotStuffPeriodFlag.Name)), developersAddr)
		// if !ctx.GlobalIsSet(MinerGasPriceFlag.Name) && !ctx.GlobalIsSet(MinerLegacyGasPriceFlag.Name) {
		// 	cfg.Miner.GasPrice = big.NewInt(1)
		// }
		...
		cfg.Genesis = core.DeveloperHotStuffGenesisBlock(uint64(ctx.GlobalInt(DeveloperHotStuffPeriodFlag.Name)), developer.Address)
		if !ctx.GlobalIsSet(MinerGasPriceFlag.Name) && !ctx.GlobalIsSet(MinerLegacyGasPriceFlag.Name) {
			cfg.Miner.GasPrice = big.NewInt(1)
		}
	// /BLS-Upgrade
