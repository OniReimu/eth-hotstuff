...
func TestCommit(t *testing.T) {
	...
	testCases := []struct {
		expectedErr           error
		expectedValSet        hotstuff.ValidatorSet
		expectedCollectionPub map[common.Address][]byte
		expectedCollectionSig map[common.Address][]byte
		expectedBlock         func() *types.Block
	}{
		// {
		// 	// TODO: normal case for Aggregate Signature
		// 	nil,
		// 	nil,
		// 	nil,
		// 	nil,
	...
	for _, test := range testCases {
		...
		if err := backend.Commit(expBlock, test.expectedValSet, test.expectedCollectionPub, test.expectedCollectionSig); err != nil {
...
func TestGetSpeaker(t *testing.T) {
	...
	if actual != expected {
		t.Errorf("speaker mismatch: have %v, want %v", actual.Hex(), expected.Hex())
	}
}
