...
const (
	dbKeySnapshotPrefix = "hotstuff-snapshot"
)
...
type Snapshot struct {
	...
	ValSet hotstuff.ValidatorSet    `json:"signers"` // Set of authorized signers at this moment

	Recents  map[uint64]common.Address `json:"recents"` // Set of recent signers for spam protections
	sigcache *lru.ARCCache             // Cache of recent block signatures to speed up ecrecover

	Processing map[common.Address]uint64 `json:"processing"` // Defer the change of validator due to the unconfirmed ones
}

// newSnapshot create a new snapshot with the specified startup parameters. This
// method does not initialize the set of recent validators, so only ever use if for
// the genesis block.
func newSnapshot(sigcache *lru.ARCCache, epoch uint64, number uint64, hash common.Hash, valSet hotstuff.ValidatorSet) *Snapshot {
	snap := &Snapshot{
		Epoch:      epoch,
		Number:     number,
		Hash:       hash,
		...
		sigcache:   sigcache,
		Processing: make(map[common.Address]uint64),
	}
	return snap
}
...
func (s *Snapshot) copy() *Snapshot {
	...
	for address, times := range s.Processing {
		cpy.Processing[address] = times
	}
...
func (s *Snapshot) apply(headers []*types.Header) (*Snapshot, error) {
	...
	for _, header := range headers {
		...
		if limit := uint64(snap.ValSet.Size()/2 + 1); number >= limit {
		...
		// Accummulate the processing times for each vote
		...
		if tally := snap.Tally[header.Coinbase]; tally.Votes > snap.ValSet.Size()/2 {
			if _, ok := snap.Processing[header.Coinbase]; !ok {
			...
			if tally.Authorize {
				...
				// Should the deauthorized validator that is being freezed within the unconfirmation window be counted? --saber
...
type snapshotJSON struct {
	...
	Policy     hotstuff.SpeakerPolicy `json:"policy"`
}
