...
func TestVoting(t *testing.T) {
	...
	for i, tt := range tests {
		...
		genesis := &core.Genesis{
			...
			Mixhash:    types.HotStuffDigest,
		}
		b := genesis.ToBlock(nil)
		extra, _ := prepareExtra(b.Header(), b.Coinbase(), validators)
		...
		headers := make([]*types.Header, len(tt.votes)+int(params.MinimumUnconfirmed))
		for j, vote := range tt.votes {
			...
			extra, _ := prepareExtra(headers[j], accounts.address(vote.validator), validators)
		...
		// TODO: Need to check the correctness. Here applied the 3-accumulation
		unconfirmHead := len(tt.votes) - 1
		for j := 0; j < int(params.MinimumUnconfirmed); j++ {
			index := unconfirmHead + j + 1
			headers[index] = &types.Header{
				Number:     big.NewInt(int64(index)),
				Time:       uint64(int64(index-1) * int64(config.BlockPeriod)),
				Coinbase:   accounts.address("A"),
				Difficulty: defaultDifficulty,
				MixDigest:  types.HotStuffDigest,
			}
			extra, _ := prepareExtra(headers[index], accounts.address("A"), validators)
			headers[index].Extra = extra
			headers[index].ParentHash = headers[index-1].Hash()
			copy(headers[index].Extra, genesis.ExtraData)
			accounts.sign(headers[index], "A")
		}
