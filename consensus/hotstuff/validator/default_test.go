...
func testNormalValSet(t *testing.T) {
	...
	// test get speaker
	if val := valSet.GetSpeaker(); !reflect.DeepEqual(val, val1) {
		t.Errorf("speaker mismatch: have %v, want %v", val, val1)
	}
	// test calculate speaker
	lastSpeaker := addr1
	valSet.CalcSpeaker(lastSpeaker, uint64(0))
	if val := valSet.GetSpeaker(); !reflect.DeepEqual(val, val2) {
		t.Errorf("speaker mismatch: have %v, want %v", val, val2)
	}
	valSet.CalcSpeaker(lastSpeaker, uint64(3))
	if val := valSet.GetSpeaker(); !reflect.DeepEqual(val, val1) {
		t.Errorf("speaker mismatch: have %v, want %v", val, val1)
	}
...
func testStickySpeaker(t *testing.T) {
	b1 := common.Hex2Bytes(testAddress)
	b2 := common.Hex2Bytes(testAddress2)
	addr1 := common.BytesToAddress(b1)
	addr2 := common.BytesToAddress(b2)
	...
	valSet.CalcSpeaker(lastSpeaker, uint64(1))
	if val := valSet.GetSpeaker(); !reflect.DeepEqual(val, val2) {
		t.Errorf("speaker mismatch: have %v, want %v", val, val2)
	}
	// test empty last speaker
