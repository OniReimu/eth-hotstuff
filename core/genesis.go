...
// BLS-Upgrade: DeveloperHotStuffGenesisBlock returns the 'geth --hsdev' genesis block.
func DeveloperHotStuffGenesisBlock(period uint64, faucet common.Address) *Genesis {
	// Override the default period to the user requested one
	config := *params.BLSChainConfigChanges
	config.HotStuff.Period = period
	config.HotStuff.Test = true

	// Assemble and return the genesis with the precompiles and faucet pre-funded
	genesis := &Genesis{
		Config: &config,
		// This setting is only for tests
		ExtraData:  append(append(make([]byte, 32), faucet[:]...), make([]byte, crypto.SignatureLength)...),
		GasLimit:   47000000,
		Difficulty: big.NewInt(1),
		Alloc: map[common.Address]GenesisAccount{
