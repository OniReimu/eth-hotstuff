...
// BLS-Upgrade: Returning various parts of the extra data field. (will check if this can be depreciated --saber)
func (h *Header) ExtraVanity() []byte { return h.Extra[:HotStuffExtraVanity] }
func (h *Header) SignerAddress() common.Address {
	return common.BytesToAddress(h.Extra[len(h.Extra)-HotStuffExtraSeal-common.AddressLength : len(h.Extra)-HotStuffExtraSeal])
}
func (h *Header) ExtraSeal() []byte {
	return h.Extra[len(h.Extra)-HotStuffExtraSeal : len(h.Extra)]
}

func (h *Header) SignerListByte() []byte {
	if h.Number.Uint64() == 0 {
		if len(h.Extra) < HotStuffExtraVanity+HotStuffExtraSeal {
			return nil
		}
		return h.Extra[HotStuffExtraVanity : len(h.Extra)-HotStuffExtraSeal]
	} else {
		if len(h.Extra) < HotStuffExtraVanity+HotStuffExtraSeal+common.AddressLength {
			return nil
		}
		return h.Extra[HotStuffExtraVanity : len(h.Extra)-HotStuffExtraSeal-common.AddressLength]
	}
}
func (h *Header) SignerList() []common.Address {
	bytesToAddrSlice := func(addr_byte []byte) []common.Address {
	...
	return bytesToAddrSlice(h.SignerListByte())
}
