...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	if chainConfig.IsHotStuff {
		...
		ProtocolVersions = hotstuffProtocol.Versions
	}
	// /BLS-Upgrade
...
func CreateConsensusEngine(ctx *node.ServiceContext, chainConfig *params.ChainConfig, config *Config, notify []string, noverify bool, db ethdb.Database) consensus.Engine {
	...
	// BLS-Upgrade: if hotstuff is requested, set it up
	if chainConfig.HotStuff != nil {
		if chainConfig.HotStuff.Epoch != 0 {
			config.HotStuff.Epoch = chainConfig.HotStuff.Epoch
		}
