...
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb ethdb.Database, cacheLimit int, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
	...
	// BLS-Upgrade: we abstract the protocol manager out from here and make it a new interface for the hotstuff handling.
	// This handler can also use the backend functions of the consensus engine.
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	// BLS-Upgrade: handling the hotstuff messages first if set.
	if handler, ok := pm.engine.(consensus.Handler); ok {
		pubKey := p.Node().Pubkey()
		addr := crypto.PubkeyToAddress(*pubKey)
		handled, err := handler.HandleMsg(addr, msg)
		// A TRUE means the message has been recognized as a hotstuff-oriented msg
		// and will ignore the following main handling. Otherwise it will still go
		// through the main handling.
		if handled {
			return err
		}
	}
...
// BLS-Upgrade: implementing the consensus.Handler interface
