...
var (
	...
	AllEthashProtocolChanges = &ChainConfig{big.NewInt(1337), big.NewInt(0), nil, false, big.NewInt(0), common.Hash{}, big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), nil, nil, new(EthashConfig), nil, nil, false}
	...
	// This configuration is intentionally not using keyed fields to force anyone
	// adding flags to the config to also have to set these fields.
	AllCliqueProtocolChanges = &ChainConfig{big.NewInt(1337), big.NewInt(0), nil, false, big.NewInt(0), common.Hash{}, big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), nil, nil, nil, &CliqueConfig{Period: 0, Epoch: 30000}, nil, false}
	...
	// BLS-Upgrade
	BLSChainConfigChanges = &ChainConfig{big.NewInt(1), big.NewInt(0), nil, false, big.NewInt(0), common.Hash{}, big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), nil, nil, nil, nil, &HotStuffConfig{Test: true, Period: 5, Suite: bn256.NewSuite()}, true}
	// /BLS-Upgrade
)
...
type ChainConfig struct {
	...
	IsHotStuff bool            `json:"isHotStuff "` // Quorum flag
	// /BLS-Upgrade
}
...
// BLS-Upgrade: HotStuffConfig is the consensus engine configs for hotstuff based sealing
type HotStuffConfig struct {
	Test          bool         `json:"test"`   // Indicating if the chain is for testing or not
	Epoch         uint64       `json:"epoch"`  // Epoch length to reset votes and checkpoint
	Period        uint64       `json:"period"` // Number of seconds between blocks to enforce
